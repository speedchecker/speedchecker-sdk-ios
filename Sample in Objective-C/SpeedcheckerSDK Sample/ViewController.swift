...
class ViewController: UIViewController, SpeedTestDelegate {
	...
	override func viewDidLoad() {
		...
		speedTest = SpeedTestController(clientID: 51, delegate: self)
	...
	func speedTestDidReceiveServers(_ servers: [SpeedTestServer]) {
		print("speedTestDidReceiveServers \(servers.count)")
	}
	
	func speedTestDidSelectServer(_ server: SpeedTestServer, withLatencyInMs latency: Int) {
		DispatchQueue.main.async {
	...
	func speedTestDownloadDidProgress(_ progress: Double, withSpeed speed: SpeedTestSpeed?) {
		DispatchQueue.main.async {
	...
	func speedTestUploadDidProgress(_ progress: Double, withSpeed speed: SpeedTestSpeed?) {
		DispatchQueue.main.async {
