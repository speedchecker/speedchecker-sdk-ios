...
- (void)speedTestDidSelectServer:(SpeedTestServer *)server withLatencyInMs:(NSInteger)latency {
	dispatch_async(dispatch_get_main_queue(), ^{
		_serverValue.text = server.domain;
		_latencyValue.text = [NSString stringWithFormat:@"%d ms", (int)latency];
	});
}
...
- (void)speedTestDownloadDidProgress:(double)progress withSpeed:(SpeedTestSpeed *)speed {
	dispatch_async(dispatch_get_main_queue(), ^{
		...
		[_downloadProgress setProgress:(float)progress];
		
		if (speed != NULL) {
			_downloadValue.text = [NSString stringWithFormat:@"%@ Mb/s", speed.descriptionInMbps];
		}
	});
}
...
- (void)speedTestUploadDidProgress:(double)progress withSpeed:(SpeedTestSpeed *)speed {
	dispatch_async(dispatch_get_main_queue(), ^{
		[_uploadProgress setProgress:(float)progress];
		
		if (speed != NULL) {
			_uploadValue.text = [NSString stringWithFormat:@"%@ Mb/s", speed.descriptionInMbps];
		}
...
- (void)speedTestDidFinishWithResult:(SpeedTestResult *)result {
	dispatch_async(dispatch_get_main_queue(), ^{
		_serverValue.text = result.server.domain;
		_latencyValue.text = [NSString stringWithFormat:@"%d ms", (int)result.latencyInMs];
		
		_downloadValue.text = [NSString stringWithFormat:@"%@ Mb/s", result.downloadSpeed.descriptionInMbps];
		_uploadValue.text = [NSString stringWithFormat:@"%@ Mb/s", result.uploadSpeed.descriptionInMbps];
		
		_ispValue.text = result.ispName;
		_ipValue.text = result.ipAddress;
	});
}
